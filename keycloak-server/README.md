...
|:white_circle: Authenticate (With OTP) |`REALM=flask-sso-auth-service; CLIENT_ID=flask-sso-auth-service-client-id; CLIENT_SECRET=gik4U0mU0xQ2EGBOEQlv8zzyUSmr7U9M; USERNAME="mccmak"; PASSWORD="mccmak"; read -p "OTP: " OTP; curl -X POST "http://192.168.68.130:8080/realms/$REALM/protocol/openid-connect/token" -H "Content-Type: application/x-www-form-urlencoded" -d "client_id=$CLIENT_ID" -d "client_secret=$CLIENT_SECRET" -d "username=$USERNAME" -d "password=$PASSWORD" -d "otp=$OTP" -d "grant_type=password"`|
